...
public class SHW_ValuationEffectiveDate extends SHW_ValuationEffectiveDateAbstract {
	...
	protected String doIt() throws Exception {
		/*
		 * Trx dbTransaction = null; dbTransaction = Trx.get("UpdateCostDetail", true);
		 * ProcessInfo M_CostDetailGenerate_Batch = ProcessBuilder.create(getCtx())
		 * .process(54385) .withTitle("M_CostDetailGenerate_Batch")
		 * .withParameter(MCostDetail.COLUMNNAME_AD_Client_ID,
		 * Env.getAD_Client_ID(getCtx())) .execute(dbTransaction.getTrxName()); if
		 * (M_CostDetailGenerate_Batch.isError()) throw new
		 * AdempiereException(M_CostDetailGenerate_Batch.getSummary()); if
		 * (dbTransaction != null) { dbTransaction.commit(true); dbTransaction.close();
		 * dbTransaction = null; }
		 */
		setup();
